...
void ads12xx::SetRegisterValue(uint8_t regAdress, uint8_t regValue) {
  ...
  if (regAdress == IDAC0) {
    regValue =
        regValue | IDAC0_ID; // add non 0 non-write register value IDAC0_ID
  }
#endif
  ...
  if (regValue != regValuePre) {
    ...
    SPI.beginTransaction(SPISettings(
        SPI_SPEED, MSBFIRST,
        SPI_MODE1)); // initialize SPI with SPI_SPEED, MSB first, SPI Mode1
    digitalWrite(_CS, LOW);
    delayMicroseconds(10);
    SPI.transfer(WREG |
                 regAdress); // send 1st command byte, address of the register
    ...
    if (regValue !=
        ads12xx::GetRegisterValue(regAdress)) { // Check if write was succesfull
...
unsigned long ads12xx::GetRegisterValue(uint8_t regAdress) {
  ...
  SPI.beginTransaction(SPISettings(
      SPI_SPEED, MSBFIRST,
      SPI_MODE1)); // initialize SPI with 4Mhz clock, MSB first, SPI Mode0
  uint8_t bufr;
  digitalWrite(_CS, LOW);
  delayMicroseconds(10);
  SPI.transfer(RREG |
...
void ads12xx::Reset() {
  SPI.beginTransaction(SPISettings(
      SPI_SPEED, MSBFIRST,
      SPI_MODE1)); // initialize SPI with  clock, MSB first, SPI Mode1
  digitalWrite(_CS, LOW);
  delayMicroseconds(10);
